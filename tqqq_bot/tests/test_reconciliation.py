...
async def test_working_sell_missing_and_shares_match(engine, mock_broker, mock_sheet):
    """
    First tick: WORKING_SELL missing, broker shares perfectly match raw tracking.
    Halt is bypassed, but the tick is skipped.
    Second tick: WORKING_SELL still missing, bypass is applied, grid logic proceeds
    to _run_pre_sell_guard and places replacement order.
    """
    engine.grid_state = GridState(rows={
        7: GridRow(row_index=7, status="WORKING_SELL:101", has_y=True, sell_price=10.0, buy_price=9.0, shares=101)
    })
    ...
    # Tick 1: Bypassed but skipped
    with patch.object(engine, '_run_pre_sell_guard', new_callable=AsyncMock) as mock_guard:
        skipped = await engine._check_reconciliation_and_halt(open_orders=open_orders, broker_shares=101)
        assert engine._halted_reconciliation is False
        assert skipped is True
        assert "101" in engine._missing_sell_confirmations
        mock_sheet.append_error.assert_not_called()

        # Tick 2: Bypassed, confirmed missing, replaced
        mock_guard.return_value = False
        skipped = await engine._check_reconciliation_and_halt(open_orders=open_orders, broker_shares=101)
        assert engine._halted_reconciliation is False
        assert skipped is False
        assert "101" not in engine._missing_sell_confirmations
        assert engine.grid_state.rows[7].status == "WORKING_SELL:101" # Does not modify tracker directly

@pytest.mark.asyncio
async def test_working_sell_missing_reappears(engine, mock_broker, mock_sheet):
    """
    First tick: WORKING_SELL missing, confirmed missing.
    Second tick: WORKING_SELL reappears in open_orders, confirmation is cleared.
    """
    engine.grid_state = GridState(rows={
        7: GridRow(row_index=7, status="WORKING_SELL:101", has_y=True, sell_price=10.0, buy_price=9.0, shares=101)
    })
...
async def test_working_buy_missing_and_shares_match(engine, mock_broker, mock_sheet):
    """
    WORKING_BUY missing must not be self-healed or altered.
    """
    engine.grid_state = GridState(rows={
    ...
    await engine._check_reconciliation_and_halt(open_orders=open_orders, broker_shares=100)
    assert engine._halted_reconciliation is False
    assert engine.grid_state.rows[8].status == "WORKING_BUY:102"
    mock_sheet.append_error.assert_not_called()

@pytest.mark.asyncio
async def test_two_tick_stale_order_recovery(engine, mock_broker, mock_sheet):
    """
    Two-tick integration test for missing SELL.
    First _tick() skips placing orders.
    ...
    with patch('app.engine.grid_state.GridState.distal_y_row', new_callable=PropertyMock, return_value=17):
        await engine._tick()

    assert engine._halted_reconciliation is False
    assert mock_broker.place_limit_order.call_count == 0
    assert engine.grid_state.rows[17].status == "WORKING_SELL:117"
    ...
    # Remove any side_effect from get_next_order_id/place_limit_order left over from the fixture mock
    mock_broker.get_next_order_id.side_effect = None
    mock_broker.place_limit_order.side_effect = None
    mock_broker.get_next_order_id.return_value = '999'
    mock_broker.place_limit_order.return_value = OrderResult(order_id='999', status='submitted')
